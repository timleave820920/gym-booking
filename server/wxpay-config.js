 ...
 *
 * 个人号阶段无商户号 → 未启用（isWxpayEnabled()=false）：
 ...
 * 绝不模拟微信支付成功——发布前必须关的洞（历史：前端调 payOrder 即标 paid）。
 *
 * 商户号到位后（公司号阶段）配置以下环境变量即启用，业务代码零改动：
...
function isWxpayEnabled() {
  return !!(process.env.WXPAY_MCH_ID && process.env.WXPAY_SERIAL_NO
...
module.exports = { isWxpayEnabled, getNotifyUrl };
