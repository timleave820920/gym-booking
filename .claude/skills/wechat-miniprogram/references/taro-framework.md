...
Taro 3.x 采用运行时架构，在运行时将 React/Vue 的渲染操作适配到小程序的视图层。

**支持平台：** 微信/支付宝/百度/字节/QQ/京东/快应用/H5/React Native
...
src/pages/index/
├── index.tsx          # 通用
├── index.weapp.tsx    # 微信小程序专用（优先级更高）
├── index.h5.tsx       # H5 专用
├── index.scss         # 通用样式
└── index.weapp.scss   # 微信专用样式
```
...
Taro 3.x 因运行时架构，长列表性能弱于原生。解决方案：

```tsx
// 使用 @tarojs/plugin-framework-react 的 CustomWrapper
...
### 微信插件处理（以 WechatSI 同声传译为例）

#### touristappid 不支持插件

使用测试 AppID（touristappid）开发时，声明 `plugins` 配置会导致运行时错误：
...
**解决方案**：注释掉 plugins 配置 + 代码中优雅降级

```ts
// app.config.ts — 使用 touristappid 时注释掉
export default defineAppConfig({
  pages: [...],
  // plugins: {
  //   WechatSI: {
...
function initPlugin(): void {
  ...
  try {
    ...
    plugin = null  // 插件不可用，静默降级
  }
}

export function playPronunciation(word: string, accent: 'UK' | 'US' = 'US'): Promise<void> {
  initPlugin()

  if (!plugin) {
    Taro.showToast({ title: '语音插件不可用，请使用真实 AppID', icon: 'none', duration: 2000 })
  ...
  return new Promise((resolve) => {
    plugin.textToSpeech({
      ...
      success: (res: { filename: string }) => {
        ...
        ctx.onError(() => { ctx.destroy(); resolve() })
        ctx.play()
      },
      fail: () => {
        Taro.showToast({ title: '发音播放失败', icon: 'none' })
        resolve()
      },
    })
  })
...
1. 修改 `project.config.json` 中的 `appid` 为真实 AppID
2. 登录 mp.weixin.qq.com → 设置 → 第三方设置 → 添加插件 → 搜索"微信同声传译"
...
export const useGameStore = create(
  persist(
    ...
    {
      name: 'game-storage',
      storage: createJSONStorage(() => wxStorage),  // ✅ 替代 localStorage
    }
  )
)
```
