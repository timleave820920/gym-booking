...
- [ ] **P1｜2. Dashboard 前端（web 运营数据 tab）** — courses.html tab「邀请看板」→「运营数据」：7 KPI 卡（环比）+ 趋势图（原生 canvas）+ 4 组折叠卡 + 响应式；FRONT 断言防回退
- [ ] **P1｜3. 用户分析：分层 + 清单 + 钻取** — `GET /api/admin/users-analysis`：RMF 分层筛选（近度/频次/金额/沉睡档位/偏好标签/搜索）+ 分页 + CSV 导出（复用 B3 导出模式）；清单点用户 → 行为时间线 + 标签卡；群组选中 → 站内信触达
