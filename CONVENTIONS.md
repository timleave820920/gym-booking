...
| 9 | **取时间必须走 `server/time.js`（北京时间）** | 业务代码取「当前时间」**一律用 `server/time.js`**（Intl 显式 Asia/Shanghai，与系统时区无关）；**禁止**裸 `new Date().getHours()/getMinutes()/getFullYear()/toLocaleString('sv-SE')` 等隐式依赖系统时区的写法；SQL 与 `datetime('now','localtime')` 比较时用 `nowDateTimeStr()` 显式传参。本地测试（Windows 北京时间）发现不了时区错位，CI/云托管（UTC）才会红——BUG-LEDGER #28 教训：签到窗口曾整体差 8 小时 | 不可绕过（review 检查 `grep -n "getHours()\\|getFullYear()\\|toLocaleString(" server/`） |

## 💼 业务规则基线（产品层约束，改前必读）

| # | 规则 | 说明 | 现状 |
|---|---|---|---|
| B1 | **签到是唯一的"实际到课"证明** | 用户成长指标（锻炼次数 total_classes/等级/勋章）只统计**签到成功**的课，订课/支付不计（没到场不算锻炼） | ✅ 已落地（checkinBooking 时 +1；2026-08-13 田立确认） |
| B2 | **教练课时提成按签到数结算，而非订课数** | 教练收入锚定"实际完成的教学"，杜绝"订了不来白拿提成"。**激励闭环**：用户想累积成长 → 要求教练必须签到；教练想要提成 → 必须如实服务并签到。签到是双方共同利益点 | ⏳ 模块未实现（后端/前端均无提成代码），**未来开发时以此为硬约束**（见 TODO） |
| B3 | **签到窗口纪律** | 开课前 30 分钟～课后 30 分钟可签（后端服务器时钟裁决，手机改时间无效）；窗口外前端不显示入口 + 后端拒绝 | ✅ 已落地（BUG-LEDGER #10；2026-08-16 统一为课后 30 分钟，DESIGN #D1） |

> 任何涉及签到/统计/教练结算的改动，必须先对照 B1-B3，不得破坏「签到=唯一到课证明」的语义。
...
| **每周五结构评估** | 每周五（cron 闹钟 + 本承诺双保险） | 按《项目审查-2026-08-16.md》4 维标准评分（流程清晰/失败可学/项目变薄/知识沉淀）+ 对照待办清单，记录追加进评审文档 |
