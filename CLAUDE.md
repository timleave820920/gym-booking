...
- **MySQL 方言坑全家桶（BUG-LEDGER #60/#61，2026-08-18 CI 全绿战役，六批收尾）**：① `LIMIT ?`/`OFFSET ?` 参数绑定——mysql2 把 number 编码为 DOUBLE，MySQL 报 ER_WRONG_ARGUMENTS，必须文本拼接 `Number()||默认`；② SUM/ROUND 返回 DECIMAL，mysql2 默认转 string（SQLite 返回 number）——`decimalNumbers:true` 连接池配置 + 业务 `fen()`/`Number()` 强转双保险；③ `MAX(a,b)` 是 SQLite 标量扩展、MySQL 用 GREATEST（SQLite 又没有）——**CASE WHEN 是唯一双方言安全写法**；④ 保留字（change/date/read/desc）必须反引号，模板字符串内 `\`` 转义；⑤ 事务必须单连接承载（MysqlDriver._txConn，BEGIN 绑定专用连接，COMMIT/ROLLBACK 释放）；⑥ run-tests 连 MySQL 时 pool 句柄阻塞进程退出 → 挂满 CI 超时，两个退出路径都要 `process.exit`。测试永远跑 SQLite，MySQL 路径靠静态断言（MYSQL-xx 正则扫描）+ CI test-mysql job 兜底。
