...
## 〇、选型复盘教训（2026-08-17 补记）

**根因溯源**：本项目数据库选型定 SQLite 时，理由是「零 npm 依赖、node:sqlite 内置、开发快」——但从未回答一个问题：**目标部署平台（微信云托管）是否支持这个存储**。云托管是 Serverless 容器：文件系统随容器销毁（闲置缩容/推送重建/扩容都换新容器），SQLite 依赖本地盘持久化 → 上线即埋雷（BUG-LEDGER #25），最终在架构定型后整仓改造迁移 MySQL。

**踩坑全链条**：选型定 SQLite → 上线发现数据全丢 → 排查确认容器不持久化 → 试 CFS 文件存储（云托管无挂载入口，弃）→ 试静态资源存储（非 POSIX 挂载，SQLite 无法运行，弃）→ 最终环境内置 MySQL（Serverless，同环境内网直连，采纳）。**三个候选方案只有 MySQL 站得住，而 MySQL 恰恰是选型阶段就该定下的答案**。

**教训（可复用）**：
1. **存储选型以目标平台为前置**：技术选型阶段先列出目标平台的存储供给（托管数据库 / 对象存储 / 文件系统挂载），数据库直接选平台托管型（MySQL/PG）；本地开发用什么只是「方言层」的事，与生产选型解耦——本次最终架构（双驱动抽象，本地 SQLite / 生产 MySQL）本就该是选型起点
2. **「本地好用」≠「生产可行」**：容器平台的文件系统语义（不持久化、不跨实例共享）与单机进程完全不同，凡依赖本地盘的存储必须验证目标平台能力后再拍板
3. **改造代价是选型失误的利息**：本次迁移成本 = 双驱动抽象层 + 313 处 DB 调用 async 化 + 89 处时间函数方言收口 + 20 张表双方言建表 + 迁移脚本。若选型期就定 MySQL，这一切归零
