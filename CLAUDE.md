...
- **云托管对接 VPC/CFS 必须用「小程序公众号」方式登录腾讯云**（2026-08-16 实操验证）：微信云托管的腾讯云资源在小程序主体绑定的账号下——用「微信」方式登录腾讯云（个人账号）建的 VPC/CFS，在云托管「对接 VPC」下拉里**永远看不到**。必须 console.cloud.tencent.com 登录方式选「**小程序公众号**」→ 微信扫码 → 选择本小程序，VPC/CFS 都在该账号下创建（微信云托管仅支持上海地域 VPC）。详见「云托管持久化与身份配置.md」。
