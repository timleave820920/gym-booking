...
学员端 + 教练端 + 管理后台全链路小程序，基于 Ardot 设计稿开发，含 SQLite 后端（用户注册/登录）。
...
**小程序**：
1. 打开**微信开发者工具** → 导入项目
2. 选择项目目录：`综合训练馆订课系统`（已含 `project.config.json`，`urlCheck` 已关闭）
3. 编译后登录页 → 微信一键登录 / 手机号快捷登录
4. 首次登录=**注册**（写入数据库），再次登录=**登录**（读库验证，登录次数递增）

> 真机预览时：后端地址改为电脑局域网 IP（`miniprogram/utils/api.js` 中 `BASE_URL`，当前电脑 IP 为 `192.168.194.11`）。
...
| 接口 | 方法 | 说明 |
|---|---|---|
| `/api/auth/login` | POST | 注册/登录（首次=注册 201，再次=登录 200，`isNewUser` 区分） |
| `/api/auth/profile` | POST | 更新用户资料（昵称/头像） |
| `/api/users` | GET | 用户列表（后台用） |
| `/api/users/stats` | GET | 用户统计 |
| `/api/health` | GET | 健康检查 |

**数据库**：SQLite（`server/data/gym.db`），用户表 `users`（openid 唯一标识、昵称、头像、手机号、角色、注册时间、最后登录、登录次数）。
