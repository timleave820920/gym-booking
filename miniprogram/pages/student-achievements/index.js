...
const ACHIEVEMENTS = [
  // ── 阶段一：第 1-5 天（密集起步）──
  { id: 1, key: 'first_book',  name: '初次见面', icon: '约', color: '#B9FF66', stage: '第 1-5 天', day: 1,
    desc: '完成首次预约订课', check: (c) => c.bookedCount >= 1 },
  { id: 2, key: 'first_workout',  name: '第一滴汗', icon: '汗', color: '#FFD166', stage: '第 1-5 天', day: 1,
    desc: '完成第 1 次锻炼', check: (c) => c.totalClasses >= 1 },
  { id: 3, key: 'first_checkin',  name: '签到首秀', icon: '签', color: '#4ECDC4', stage: '第 1-5 天', day: 1,
    desc: '完成第 1 次签到', check: (c) => c.checkedIn >= 1 },
  { id: 4, key: 'streak_3',  name: '三日不辍', icon: '三', color: '#FF8C42', stage: '第 1-5 天', day: 3,
    desc: '连续锻炼 3 天', check: (c) => c.streak >= 3 },
  { id: 5, key: 'streak_5',  name: '五日坚持', icon: '五', color: '#B9FF66', stage: '第 1-5 天', day: 5,
    desc: '连续锻炼 5 天', check: (c) => c.streak >= 5 },

  // ── 阶段二：第 2-4 周 ──
  { id: 6, key: 'total_10',  name: '十日之基', icon: '十', color: '#4ECDC4', stage: '第 2-4 周', day: 14,
    desc: '累计锻炼 10 次', check: (c) => c.totalClasses >= 10 },
  { id: 7, key: 'streak_7',  name: '七日满贯', icon: '七', color: '#5B57EB', stage: '第 2-4 周', day: 21,
    desc: '连续锻炼 7 天', check: (c) => c.streak >= 7 },
  { id: 8, key: 'streak_14',  name: '双周不辍', icon: '双', color: '#185FA5', stage: '第 2-4 周', day: 28,
    desc: '连续锻炼 14 天', check: (c) => c.streak >= 14 },

  // ── 阶段三：第 1-3 个月 ──
  { id: 9, key: 'member_30',  name: '满月之约', icon: '月', color: '#7F77DD', stage: '第 1-3 个月', day: 42,
    desc: '成为会员满 30 天', check: (c) => c.memberDays >= 30 },
  { id: 10, key: 'total_20', name: '二十次历练', icon: '廿', color: '#534AB7', stage: '第 1-3 个月', day: 56,
    desc: '累计锻炼 20 次', check: (c) => c.totalClasses >= 20 },
  { id: 11, key: 'total_30', name: '百日筑基', icon: '百', color: '#3C3489', stage: '第 1-3 个月', day: 84,
    desc: '累计锻炼 30 次', check: (c) => c.totalClasses >= 30 },

  // ── 阶段四：第 2-4 季度（固定节奏 · 每 2 周一项）──
  { id: 12, key: 'total_40', name: '四十次淬炼', icon: '四', color: '#D85A30', stage: '第 2-4 季度', day: 98,
    desc: '累计锻炼 40 次', check: (c) => c.totalClasses >= 40 },
  { id: 13, key: 'member_60', name: '双月笃行', icon: '双', color: '#BA7517', stage: '第 2-4 季度', day: 112,
    desc: '成为会员满 60 天', check: (c) => c.memberDays >= 60 },
  { id: 14, key: 'first_recharge', name: '储值先行', icon: '储', color: '#F8D044', stage: '第 2-4 季度', day: 126,
    desc: '完成首次储值', check: (c) => c.rechargeFen > 0 },
  { id: 15, key: 'invite_1', name: '引伴同行', icon: '伴', color: '#D85A30', stage: '第 2-4 季度', day: 140,
    desc: '成功邀请 1 位好友', check: (c) => c.inviteCount >= 1 },
  { id: 16, key: 'kinds_4', name: '多面手', icon: '手', color: '#7F77DD', stage: '第 2-4 季度', day: 154,
    desc: '体验 4 种不同课程', check: (c) => c.courseKinds >= 4 },
  { id: 17, key: 'total_60', name: '六十次磨砺', icon: '六', color: '#E5484D', stage: '第 2-4 季度', day: 168,
    desc: '累计锻炼 60 次', check: (c) => c.totalClasses >= 60 },
  { id: 18, key: 'coins_200', name: '能量新星', icon: '能', color: '#B9FF66', stage: '第 2-4 季度', day: 182,
    desc: '能量币余额达 200', check: (c) => c.coins >= 200 },
  { id: 19, key: 'invite_3', name: '老友记', icon: '友', color: '#5B57EB', stage: '第 2-4 季度', day: 196,
    desc: '成功邀请 3 位好友', check: (c) => c.inviteCount >= 3 },
  { id: 20, key: 'total_80', name: '八十次突破', icon: '八', color: '#D85A30', stage: '第 2-4 季度', day: 210,
    desc: '累计锻炼 80 次', check: (c) => c.totalClasses >= 80 },
  { id: 21, key: 'hours_50', name: '单课宗师', icon: '时', color: '#BA7517', stage: '第 2-4 季度', day: 224,
    desc: '累计锻炼时长 50 小时', check: (c) => c.totalMinutes >= 3000 },
  { id: 22, key: 'total_100', name: '百炼成钢', icon: '钢', color: '#A32D2D', stage: '第 2-4 季度', day: 238,
    desc: '累计锻炼 100 次', check: (c) => c.totalClasses >= 100 },
  { id: 23, key: 'coins_500', name: '能量达人', icon: '力', color: '#B9FF66', stage: '第 2-4 季度', day: 252,
    desc: '能量币余额达 500', check: (c) => c.coins >= 500 },
  { id: 24, key: 'total_120', name: '一百二十次', icon: '贰', color: '#D85A30', stage: '第 2-4 季度', day: 266,
    desc: '累计锻炼 120 次', check: (c) => c.totalClasses >= 120 },
  { id: 25, key: 'streak_30', name: '连续三十', icon: '连', color: '#185FA5', stage: '第 2-4 季度', day: 280,
    desc: '连续锻炼 30 天', check: (c) => c.streak >= 30 },
  { id: 26, key: 'recharge_2000', name: '储值升级', icon: '金', color: '#F8D044', stage: '第 2-4 季度', day: 294,
    desc: '累计储值 ¥2000', check: (c) => c.rechargeFen >= 200000 },
  { id: 27, key: 'total_150', name: '一百五十次', icon: '伍', color: '#E5484D', stage: '第 2-4 季度', day: 308,
    desc: '累计锻炼 150 次', check: (c) => c.totalClasses >= 150 },
  { id: 28, key: 'member_270', name: '三季之约', icon: '季', color: '#BA7517', stage: '第 2-4 季度', day: 322,
    desc: '成为会员满 270 天', check: (c) => c.memberDays >= 270 },
  { id: 29, key: 'hours_100', name: '双百小时', icon: '百', color: '#534AB7', stage: '第 2-4 季度', day: 350,
    desc: '累计锻炼时长 100 小时', check: (c) => c.totalMinutes >= 6000 },

  // ── 阶段五：一整年 ──
  { id: 30, key: 'member_365', name: '周年之约', icon: '年', color: '#F8D044', stage: '一整年', day: 364,
...
Page({
  ...
  loadAchievements() {
    ...
    // 并行拉取：订课 / 会员(注册天数+能量币) / 充值 / 邀请 / 成就同步（解锁即发 50 能量币）
    Promise.all([
      ...
      api.syncAchievements(openid).catch(() => null)
    ]).then(([bkRes, lvRes, reRes, invRes, syncRes]) => {
      ...
      // 30 项成就判定：以服务端 sync 解锁记录为权威（防前端口径偏差），sync 不可用时本地 check 兜底
      const syncKeys = new Set((syncRes && syncRes.unlockedKeys) || []);
      const enriched = ACHIEVEMENTS.map(a => ({
        ...a,
        unlocked: syncKeys.size ? syncKeys.has(a.key) : !!a.check(ctx)
      }));

      // 新解锁提示（解锁成就 +50 能量币 × N）
      if (syncRes && syncRes.newly && syncRes.newly.length) {
        const reward = syncRes.reward || 50;
        wx.showToast({
          title: `解锁成就 ${syncRes.newly.length} 项，+${reward * syncRes.newly.length} 能量币`,
          icon: 'none',
          duration: 2500
        });
      }
